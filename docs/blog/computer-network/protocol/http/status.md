...
缓存过期后向服务器发起请求验证缓存是否有效，有效的话则返回 304。304 多出现在对于静态资源的请求上面。对于静态资源来说：

当浏览器第一次请求该资源时（请求头中没有携带 If-Modified-Since），server 会在响应中告诉浏览器这个资源最后修改的时间（响应头中的 Last-Modified）。当你再次请求这个资源时，浏览器会询问 server 这个资源有没有被修改（请求头中携带 If-Modified-Since）。
...
为了解决这些问题，[ETag](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Caching#etagif-none-match) 响应标头被标准化作为替代方案。
